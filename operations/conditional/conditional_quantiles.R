...
# This file registers the conditional quartiles operation.
# It calculates the quartiles of one column for each group of a second column.
register_operation(
  ...
  fn = function(df, cols) {
    ...
    # Calculate the quartiles of values for each group in groups.
    grouped_stats <- tapply(values[complete_rows], groups[complete_rows], quantile, na.rm = TRUE)
