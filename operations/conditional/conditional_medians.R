...
# This file registers the conditional medians operation.
# It calculates the median of one column for each group of a second column.
register_operation(
  ...
  fn = function(df, cols) {
    ...
    # Calculate the median of values for each group in groups.
    grouped_stats <- tapply(values[complete_rows], groups[complete_rows], median, na.rm = TRUE)
