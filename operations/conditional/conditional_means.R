...
# This file registers the conditional means operation.
# It calculates the mean of one column for each group of a second column.
register_operation(
  ...
  fn = function(df, cols) {
    ...
    # Calculate the mean of values for each group in groups.
    grouped_stats <- tapply(values[complete_rows], groups[complete_rows], mean, na.rm = TRUE)
