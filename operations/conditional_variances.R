...
# This file registers the conditional variances operation.
# It calculates the variance of one column for each group of a second column.
register_operation(
  ...
  fn = function(df, cols) {
    ...
    # Calculate the variance of values for each group in groups.
    grouped_stats <- tapply(values[complete_rows], groups[complete_rows], var, na.rm = TRUE)
