...
# This file registers the conditional modes operation.
# It calculates the mode of one column for each group of a second column.
register_operation(
  ...
  fn = function(df, cols) {
    ...
    # Calculate the mode of values for each group in groups.
    # stat_mode is a shared helper function, defined in another file.
    grouped_stats <- tapply(values[complete_rows], groups[complete_rows], stat_mode)
