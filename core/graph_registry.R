...
register_graph <- function(name, fn, types, min_cols = 1, max_cols = 1,

                            render = "plot", params = list(), colours = FALSE,

                            regression_toggle = FALSE) {

  stopifnot(is.function(fn), length(formals(fn)) >= 3,

            is.logical(colours), length(colours) == 1,

            is.logical(regression_toggle), length(regression_toggle) == 1)

  invalid_specs <- vapply(params, function(spec) {

    !is.list(spec) || !all(c("id", "type", "label", "default") %in% names(spec))

  }, logical(1))

  if (any(invalid_specs)) {

    stop("Graph '", name, "': every params entry must be created with ",
  ...
  .graph_registry$graphs[[name]] <- list(
    ...
    render = render, params = params, colours = colours,
...
local({
  ...
  for (graph_file in list.files(graph_dir, pattern = "\\.R$", full.names = TRUE, recursive = TRUE)) {
...
applicable_graphs <- function(selected_types) {
  ...
  fits_selection <- vapply(available_graphs, function(graph) {
    ...
    n_selected >= graph$min_cols && n_selected <= graph$max_cols && all(selected_types %in% graph$types)
