...
discover_modules <- function(dir = NULL, refresh = FALSE) {
  if (!refresh && !is.null(.module_cache$modules)) return(.module_cache$modules)

  if (is.null(dir)) dir <- app_file("modules")
  if (!dir.exists(dir)) stop("Modules folder not found; no modules loaded.")
  ...
  mods <- lapply(files, function(mod_file) {
    ...
    ui <- get0(paste0("mod_", id, "_ui"), envir = mod_env, inherits = FALSE)
    server <- get0(paste0("mod_", id, "_server"), envir = mod_env, inherits = FALSE)
    # The registry skips a file that lacks one of the two functions.
    if (is.null(ui) || is.null(server)) {
      warning("Skipping '", mod_file, "': missing mod_", id, "_ui or mod_", id, "_server.")
      return(NULL)
    }
    ...
    list(
      ...
      ui = ui,
      server = server,
      where = rlang::`%||%`(get0(paste0("mod_", id, "_where"), envir = mod_env, inherits = FALSE), "sidebar"),
      order = rlang::`%||%`(get0(paste0("mod_", id, "_order"), envir = mod_env, inherits = FALSE), Inf)
    )
  })
  ...
  orders <- vapply(mods, function(module) module$order, numeric(1))
  ids <- vapply(mods, function(module) module$id, character(1))
...
build_module_ui <- function(where) {
  mods <- Filter(function(module) module$where == where, discover_modules())
  unclass(do.call(tagList, lapply(mods, function(module) module$ui(module$id))))
}

# This function starts the server of every module.
start_module_servers <- function(store) {
  for (module in discover_modules()) module$server(module$id, store)
  invisible(NULL)
}
