...
t_sprintf <- function(fmt, ...) {
  fmt <- i18n$t(fmt)
  tryCatch(sprintf(fmt, ...), error = function(e) fmt)
}
...
render_param <- function(spec, value, ns) {
  ...
  switch(spec$type,
    num = {
      args <- list(inputId = id, label = label, value = value)
      if (is.finite(spec$min)) args$min <- spec$min
      if (is.finite(spec$max)) args$max <- spec$max
      if (!is.na(spec$step)) args$step <- spec$step
  ...
  values <- lapply(param_specs, function(spec) {
    ...
    switch(spec$type,
      num = {
        ...
        if (length(value) != 1 || is.na(value)) spec$default else max(spec$min, min(spec$max, value))
      },
      if (is.null(raw) || (is.character(raw) && !nzchar(raw))) spec$default else raw
    )
  })
  setNames(values, vapply(param_specs, function(spec) spec$id, character(1)))
}

# This helper adds the resolution, the colours, and the regression flag to the parameters.
attach_graph_state <- function(params, graph_def, resolution, colour_vec = character(0),
                               show_regression = FALSE) {
  params$resolution <- resolution
  if (isTRUE(graph_def$colours)) params$colours <- colour_vec
  if (isTRUE(graph_def$regression_toggle)) params$regression_line <- show_regression
  params
}
...
graph_colours <- function(colours, n_colours) {
  if (length(colours)) colours else grDevices::hcl.colors(max(n_colours, 1))
}
