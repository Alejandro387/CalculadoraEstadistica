...
regression_label <- function(showing) {
  i18n$t(if (showing) "Hide regression line" else "Show regression line")
}
...
graph_modal_body <- function(render, ns, cols_pair = NULL) {
  switch(render,
    ...
    plot  = if (!is.null(cols_pair)) tags$div(class = "graph-plot-wrap",
                plotOutput(ns("graph_plot"), width = "100%"),
                plotOutput(ns("graph_plot_rev"), width = "100%")
              ) else tags$div(class = "graph-plot-wrap",
                     plotOutput(ns("graph_plot"), width = "100%")),
...
build_modal_controls <- function(graph_def, values, ns, resolution, regression_shown) {
  controls <- tagList()
  if (length(graph_def$params)) {
    controls <- tagList(
      controls,
      lapply(graph_def$params, function(param_spec) render_param(param_spec, values[[param_spec$id]], ns))
    )
  }
  if (isTRUE(graph_def$colours)) {
    controls <- tagList(
      ...
      tags$hr(),
      textInput(
        ns("colour_vec_text"), i18n$t("Colours"),
        placeholder = "#1f77b4, #d62728, #2ca02c"
      ),
  ...
  if (identical(graph_def$render, "plot")) {
    controls <- tagList(
      controls,
      actionButton(ns("download_plot"), i18n$t("Download"), width = "100%")
    )
  }
  controls
}
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$graph_choices <- renderUI({
      ...
      translated_radio_input(
        ns, "graph", "Choose a graph", valid_graphs(),
    ...
    run_current_graph <- function(rev_cols = FALSE) {
      ...
      tryCatch(
        run_graph(graph_data(), current$name,
                  if (rev_cols) rev(current$cols) else current$cols, params),
        error = function(e) stop(i18n$t("Error while drawing the graph"), ": ",
    ...
    width = function() preview_dims()[1],
    height = function() preview_dims()[2])

    # Draw a second plot with the two selected columns in reverse order.
    output$graph_plot_rev <- renderPlot({
      current <- spec()
      req(current, current$render == "plot", length(current$cols) == 2)
    ...
    output$graph_table <- renderTable({
      ...
      run_current_graph() |>
        format_table_columns(function(col) {
          format(col, scientific = FALSE, drop0trailing = TRUE, trim = TRUE)
        })
    }, digits = NULL)
    ...
    refresh_spec <- function() {
      ...
      req(current)
      spec(list(
        name = current$name, cols = current$cols,
        params = current_params(), render = current$render
      ))
    }
    ...
    show_graph_modal <- function(name, graph_def, values) {
      showModal(modalDialog(
        title = i18n$t(name),
        tags$style(paste(MODAL_SIZING_CSS, GRAPH_MODAL_CSS, sep = "\n")),
        tags$div(
          class = "graph-modal-grid",
          card(card_body(
            build_modal_controls(graph_def, values, ns, resolution(), show_regression())
          )),
          graph_modal_body(graph_def$render, ns,
            if (identical(graph_def$render, "plot") && length(spec()$cols) == 2)
              spec()$cols else NULL)
        ),
        easyClose = TRUE,
    ...
    current_params <- function() {
      ...
      attach_graph_state(
        collect_params(graph_def$params, input), graph_def,
        resolution(), colour_vec(), show_regression()
      )
    }
    ...
    observeEvent(input$draw, {
      ...
      reset_modal_state()
      params <- attach_graph_state(default_params(graph_def$params), graph_def,
                                   default_resolution)
      spec(list(name = graph, cols = cols, params = params, render = graph_def$render))
    ...
    observeEvent(input$set_colour_vec, {
      ...
      if (parsed$ok) {
        ...
        colour_vec_msg(
          if (!length(parsed$colours)) i18n$t("Colour vector is empty")
          else t_sprintf("Colour vector set (%d colours)", length(parsed$colours))
        )
      } else {
        colour_vec_msg(parsed$msg)
      }
      if (!is.null(spec())) refresh_spec()
    })
    ...
    observeEvent(input$append_colour, {
      ...
      value <- if (nzchar(current)) paste(current, picked, sep = ", ") else picked
    ...
    observeEvent(input$toggle_regression, {
      ...
      updateActionButton(
        session, "toggle_regression",
        label = regression_label(show_regression())
      )
      refresh_spec()
    })
    ...
    observeEvent(input$download_plot, {
      ...
      if (!apply_resolution()) return()

      file <- tempfile(pattern = "plot-", tmpdir = download_dir, fileext = ".png")
      ...
      session$sendCustomMessage(
        "openGraphTab",
        file.path(download_prefix, basename(file))
      )
    })
  })
}
