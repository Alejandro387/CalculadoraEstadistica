...
mod_type_summary_ui <- function(id) {
  ns <- NS(id)
  card(
    card_header(i18n$t("Column Selection")),
    card_body(
      uiOutput(ns("col_type_checkboxes"))
    )
  )
}
...
mod_type_summary_server <- function(id, store) {
  moduleServer(id, function(input, output, session) {
    ...
    output$col_type_checkboxes <- renderUI({
      ...
      fluidRow(
        lapply(names(type_labels), function(type) {
          cols <- names(col_types[col_types == type])
          column(
            width = 4,
            card(
              card_header(type_labels[type]),
              card_body(
                if (length(cols) == 0) {
    ...
    observe({
      selected_columns <- c(input$check_discrete, input$check_continuous, input$check_nominal)
      store$selected_cols(if (is.null(selected_columns)) character(0) else selected_columns)
    })
  })
}
