  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$operation_choices <- renderUI({
      ...
      translated_radio_input(
        ns, "operation", "Choose an operation", available_ops(),
    ...
    output$operation_params <- renderUI({
      ...
      if (is.null(param_ui)) return(NULL)
      param_ui(ns)
    })
    ...
    decimals <- reactive({
      ...
      if (length(n_decimals) != 1 || is.na(n_decimals) || n_decimals < 0) last_decimals() else n_decimals
    })
    ...
    result_table <- reactive({
      ...
      format_table_columns(
        as_result_table(value),
        function(col) format(round(col, n_decimals), trim = TRUE, scientific = FALSE)
      )
    })
    ...
    output$op_result <- renderUI({
      ...
      if (is.null(value)) return(NULL)
      if (is_scalar_result()) {
        if (is.numeric(value)) value <- round(value, n_decimals)
    ...
    observeEvent(input$proceed, {
      ...
      if (is.null(operation_name) || !(operation_name %in% available_ops())) {
        info_modal(
      ...
      params <- if (is.null(parse_param)) list() else parse_param(input)

      result(run_operation(store$df, operation_name, cols, params))
      showModal(modalDialog(
        title = i18n$t(operation_name),
        tags$style(MODAL_SIZING_CSS),
        tags$p(style = "color: #777; margin-bottom: 1em;",
               paste(cols, collapse = ", ")),
        fluidRow(
          column(width = 4,
                 numericInput(ns("decimals"), i18n$t("Decimal places"),
                              value = last_decimals(), min = 0, max = 15, step = 1)
          )
        ),
